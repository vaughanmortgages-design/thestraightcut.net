...
function partnerFeatures() {
  return data.partners.map((partner, index) => {
    const sectionId = index === 0
      ? "featured-deals"
      : `partner-${partner.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
    const imageId = index === 0 ? ` id="dogs"` : "";
    return `<section id="${sectionId}" class="pet-partner-feature"><div${imageId} class="pet-partner-image" role="img" aria-label="Dog enjoying an outdoor walk"></div><div class="pet-partner-copy"><span class="section-kicker">${index === 0 ? "Featured deal · " : ""}Approved partner</span><h2>${esc(partner.name)}</h2><p>${esc(partner.description)}</p>${partner.couponCode ? `<span class="coupon-badge">Use code ${esc(partner.couponCode)}</span>` : ""}${disclosure}<a class="button gold" href="${esc(partner.affiliateUrl)}" target="_blank" rel="sponsored nofollow noopener">${esc(partner.cta)} <span aria-hidden="true">↗</span></a></div></section>`;
  }).join("");
}
