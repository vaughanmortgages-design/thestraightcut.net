...
const HOME_DEPARTMENTS = [
  ['Electronics', 'electronics.html', 'photo-1498049794561-7780e7231661'],
  ['Home & Kitchen', 'home.html', 'photo-1556911220-bff31c812dba'],
  ['Tools & Workshop', 'tools.html', 'photo-1586864387967-d02ef85d93e8'],
  ['Automotive', 'auto.html', 'photo-1492144534655-ae79c964c9d7'],
  ['Sports & Outdoors', 'sports-outdoors.html', 'photo-1538805060514-97d9cc17730c'],
  ['Pets', 'pets.html', 'photo-1450778869180-41d0601e046e'],
...
function homepage() {
  ...
  const depts = HOME_DEPARTMENTS.map(([name, href, image]) =>
    `<a class="department-tile" href="${href}"><img loading="lazy" src="${imageUrl(image, 640)}" alt=""><span><small>Department</small><strong>${esc(name)}</strong><em>Explore →</em></span></a>`
  ).join('');
  ...
  const commerceSection = (status, icon, title, intro, href) => `<section class="section commerce-section ${status === 'new' ? 'light' : status === 'deal' ? 'warm' : 'ink'}" data-commerce-section="${status}"><div class="section-heading split"><div><span class="section-kicker">${icon} Daily Shopping Edit</span><h2>${title}</h2><p>${intro}</p></div><a class="text-link" href="${href}">Browse the full edit →</a></div><div class="product-grid" data-product-grid="${status}" aria-live="polite"></div><div class="commerce-fallback" data-commerce-fallback="${status}"><p>Fresh verified products will appear here automatically from the daily product feed.</p><a class="button ${status === 'clearance' ? 'gold' : 'dark'}" href="${href}">Explore ${title}</a></div></section>`;
  const heroSlides = [
    ['Best Deal of the Day', 'Today’s strongest verified value will appear here automatically.', 'deals.html', 'photo-1607082349566-187342175e2f'],
    ['Featured Clearance Pick', 'A final-look find worth seeing before it leaves the edit.', 'clearance.html', 'photo-1472851294608-062f824d29cc'],
    ['Featured New Arrival', 'Something new, useful and worth a closer look.', 'hot-finds.html', 'photo-1441986300917-64674bd600d8'],
  ].map(([label, title, href, image], index) => `<article class="hero-slide${index === 0 ? ' active' : ''}" data-hero-slide style="--hero:url('${imageUrl(image, 1800)}')" aria-hidden="${index !== 0}"><div class="hero-shade"></div><div class="home-hero-copy"><span class="eyebrow">${label}</span><h1>${title}</h1><p>The Straight Cut combines daily shopping inspiration with useful context and approved merchant destinations.</p><div class="hero-actions"><a class="button gold" href="${href}">Explore the edit</a><a class="button glass" href="#departments">Browse departments</a></div></div></article>`).join('');
  return `${head({ title: 'The Straight Cut | Shop Smarter. Buy Better.', description: 'A premium online department store and daily shopping publication for smarter browsing across home, technology, travel, lifestyle and more.', canonical: `${SITE}/`, image: heroImage })}<body>${header()}<main id="main"><section class="commerce-hero" data-commerce-hero>${heroSlides}<div class="hero-pagination" aria-label="Featured shopping stories"><button type="button" class="active" data-hero-dot="0" aria-label="Show best deal"></button><button type="button" data-hero-dot="1" aria-label="Show clearance pick"></button><button type="button" data-hero-dot="2" aria-label="Show new arrival"></button></div><a class="scroll-cue" href="#clearance-daily">Keep scrolling <span>↓</span></a></section><div id="clearance-daily">${commerceSection('clearance', '🔥', 'Clearance', 'Final-look products and end-cap finds, shown only when the price and destination are supplied by the verified feed.', 'clearance.html')}</div>${commerceSection('new', '🆕', 'New Arrivals', 'Recently added products across the store, organized for discovery rather than noise.', 'hot-finds.html')}${commerceSection('deal', '💰', 'Today’s Deals', 'Timely products with feed-supplied pricing and approved tracked destinations.', 'deals.html')}<section id="today" class="section light"><div class="section-heading split"><div><span class="section-kicker">Today at The Straight Cut</span><h2>Keep browsing.</h2></div><a class="text-link" href="deals.html">See all deals →</a></div><div class="feature-grid">${features}</div></section><section class="marquee" aria-label="Store highlights"><span>NEW DISCOVERIES</span><i>◆</i><span>SMARTER UPGRADES</span><i>◆</i><span>WEEKEND ESCAPES</span><i>◆</i><span>STRAIGHT RECOMMENDATIONS</span></section><section class="section ink"><div class="section-heading"><span class="section-kicker">Editor’s Choice</span><h2>Three ways to spend smarter.</h2><p>Budget edits are planning collections, not price promises. Always verify the live merchant price before purchasing.</p></div><div class="editorial-grid">${budget}</div></section><section id="departments" class="section light"><div class="section-heading split"><div><span class="section-kicker">The Department Store</span><h2>Every aisle is open.</h2><p>Finished destinations with editorial collections, buying guidance and useful next steps.</p></div><a class="text-link" href="departments.html">View all departments →</a></div><div class="department-grid">${depts}</div></section><section class="travel-feature" style="--travel:url('${imageUrl('photo-1500530855697-b586d89ba3ee', 1800)}')"><div><span class="eyebrow">Travel Picks</span><h2>Leave room for somewhere new.</h2><p>Weekend inspiration, hotel ideas, vacation homes and elevated planning through approved travel partners.</p><a class="button gold" href="travel.html">Explore Travel & Getaways</a></div></section><section class="section warm"><div class="section-heading"><span class="section-kicker">Plan the Getaway</span><h2>Approved travel partners.</h2></div>${disclosure()}<div class="partner-grid">${travelCards}</div></section><section class="section light gift-section"><div class="section-heading"><span class="section-kicker">Gift Ideas & Seasonal Collections</span><h2>Good ideas for the people on your list.</h2><p>Browse by interest and occasion without made-up urgency, fake ratings or unverified prices.</p></div><div class="editorial-grid">${editorialCard('The Host Gift Edit', 'Useful, polished ideas for dinners, weekends away and thoughtful thank-yous.', 'gifts.html', 'Browse gift ideas')}${editorialCard('For the Homebody', 'Comfort, calm and small luxuries for a very good night in.', 'home.html', 'Explore Home & Kitchen')}${editorialCard('For the Weekend Explorer', 'Travel inspiration and practical pre-trip reading for the next quick escape.', 'travel.html', 'Plan a getaway')}</div></section>${newsletter()}</main>${footer()}<script src="assets/store.js"></script></body></html>`;
}
